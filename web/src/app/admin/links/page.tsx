...
import { api, ApiError, BulkAction, BulkLinkInput, ImportReport, LinkRecord, LinkSort, LinkStatusFilter, TagStat } from "@/lib/api-client";
import { downloadCsvText } from "@/lib/csv";
import { parseTags } from "@/lib/tags";

const PAGE_SIZE = 20;

const SORT_OPTIONS: Array<{ value: LinkSort; label: string }> = [
  { value: "created_at_desc", label: "最新创建" },
  { value: "created_at_asc", label: "最早创建" },
  { value: "clicks_desc", label: "点击最多" },
  { value: "clicks_asc", label: "点击最少" },
  { value: "alias_asc", label: "别名 A → Z" },
  { value: "alias_desc", label: "别名 Z → A" },
];

const STATUS_OPTIONS: Array<{ value: LinkStatusFilter; label: string }> = [
...
const BULK_ACTIONS: Array<{ value: BulkAction; label: string; needsValue: "none" | "tag" | "date" }> = [
  { value: "disable", label: "停用", needsValue: "none" },
  { value: "enable", label: "启用", needsValue: "none" },
  { value: "delete", label: "删除", needsValue: "none" },
  { value: "tag", label: "打标签", needsValue: "tag" },
  { value: "untag", label: "去标签", needsValue: "tag" },
  { value: "set_expiry", label: "设置过期", needsValue: "date" },
];
...
/** "检查 200" / "检查 失败" — only shown once the link has been checked. */
function checkLabel(link: LinkRecord) {
  if (!link.last_checked_at) return "";
  return link.last_status_code ? `检查 ${link.last_status_code}` : "检查 失败";
}
...
export default function LinksPage() {
  ...
  }, [page, sort, status, tag, debouncedSearch]);
  ...
  async function runBulk() {
    ...
    if (bulkAction === "delete" && !confirm(`确认删除选中的 ${ids.length} 条链接？`)) return;
    ...
    try {
      ...
      if (result.affected < ids.length) {
        setError(`${ids.length - result.affected} 条未处理`);
      }
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold">链接管理</h1>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          />
          <button className="btn-secondary" disabled={busy} onClick={() => fileInput.current?.click()}>导入 CSV</button>
          <button className="btn-secondary" disabled={busy} onClick={exportCsv}>导出 CSV</button>
          <Link href="/admin/links/new" className="btn-primary">创建链接</Link>
        </div>
      </div>
      ...
      <div className="panel flex flex-wrap gap-3 p-4">
        ...
        {tags.length > 0 && (
          <select className="field-control w-auto" value={tag} onChange={(event) => { setTag(event.target.value); setPage(0); }}>
            ...
            {tags.map((item) => <option key={item.name} value={item.name}>{item.name}（{item.links}）</option>)}
          </select>
        )}
      ...
      {report && (
        <div className="panel space-y-3 p-4">
          <p className="text-sm">
            导入成功 <span className="font-semibold tabular-nums">{report.created}</span> 条
            {report.failed > 0 && <>，失败 <span className="font-semibold tabular-nums text-red-700">{report.failed}</span> 条</>}
          </p>
          {report.errors.length > 0 && (
            <ul className="mobile-scroll max-h-48 space-y-1 overflow-y-auto text-xs text-[var(--muted)]">
              {report.errors.map((item) => (
                <li key={item.line}>
                  第 {item.line} 行{item.alias ? `（${item.alias}）` : ""}：{item.reason}
                </li>
              ))}
            </ul>
          )}
      ...
      {selected.size > 0 && (
        <div className="panel flex flex-wrap items-center gap-3 p-4">
          <span className="text-sm">已选 <span className="font-semibold tabular-nums">{selected.size}</span> 条</span>
          <select
          ...
          {bulkNeedsValue === "tag" && (
            <input className="field-control w-auto" placeholder="标签" value={bulkValue} onChange={(event) => setBulkValue(event.target.value)} />
          )}
          {bulkNeedsValue === "date" && (
            <input type="date" className="field-control w-auto" value={bulkValue} onChange={(event) => setBulkValue(event.target.value)} />
          )}
          <button className="btn-primary" disabled={busy} onClick={runBulk}>执行</button>
          <button className="btn-secondary" disabled={busy} onClick={() => setSelected(new Set())}>取消选择</button>
        </div>
      )}
        ...
        <div className="mobile-scroll">
                ...
                <tr key={link.id}>
                  ...
                  <td className="px-5 py-4">
                    ...
                    {link.last_checked_at && (
                      <span className="mt-1 block text-xs text-[var(--muted)]">{checkLabel(link)}</span>
                    )}
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex justify-end gap-2">
                      <Link href={`/admin/links/${link.id}/edit`} className="btn-secondary">编辑</Link>
                      <button className="btn-secondary" onClick={() => setQrLink(link)}>二维码</button>
          ...
          {!loading && !links.length && (
            <p className="px-5 py-12 text-center text-sm text-[var(--muted)]">
              {debouncedSearch || status !== "all" || tag ? "没有匹配的链接。" : "暂无链接，先创建一个吧。"}
            </p>
          )}
        </div>
      </div>
