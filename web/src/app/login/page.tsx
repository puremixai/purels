...
export default function LoginPage() {
  ...
  if (checkingSession) {
    return <main className="grid min-h-[100dvh] place-items-center bg-[var(--canvas)] px-5">
      <section className="panel w-full max-w-md p-6 text-center text-sm text-[var(--muted)]">
        {t("common.loading")}
      </section>
    </main>;
  }
  ...
  return <main className="relative grid min-h-[100dvh] place-items-center overflow-hidden bg-[var(--canvas)] px-5 py-6">
    ...
    <section className="panel relative z-10 w-full max-w-md p-6 sm:p-7">
      ...
      {!verifying && providers.length > 0 && <div className="mt-4">
        <p className="mb-3 text-center text-xs tracking-wider text-[var(--muted)]">{t("login.or")}</p>
      ...
      {!verifying && <p className="mt-4 flex items-center justify-center gap-1 text-sm text-[var(--muted)]"><span>{t("login.noAccount")}</span><Link className="text-[var(--brand)]" href="/register">{t("login.register")}</Link></p>}
