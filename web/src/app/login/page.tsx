...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await api.auth.login(username, password); router.replace("/admin"); }
  ...
  return <main className="grid min-h-screen place-items-center bg-[var(--canvas)] px-5">
    <section className="panel w-full max-w-md p-8">
      ...
      <p className="mt-6 text-center text-sm text-[var(--muted)]">没有账号？<Link className="text-[var(--brand)]" href="/register">注册</Link></p>
    </section>
  </main>;
}
