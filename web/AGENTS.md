...
- Toast 只用于非阻塞的操作结果（保存、复制、导入导出、删除）；同一时刻最多显示 3 条，必须可手动关闭，并提供 `role=status` 或 `role=alert`。
- Dialog 只用于二维码、敏感凭据和需要明确确认的高风险操作；必须支持 Escape、点击遮罩关闭、焦点回收、焦点循环和明确的关闭按钮。
- 表单校验、权限、网络错误和空状态优先就地显示；不可用 Toast 替代错误源附近的恢复说明。
